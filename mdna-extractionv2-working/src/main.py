...
from .core.extractor import MDNAExtractor
...
def main():
    """Main function with in-place table preservation for MD&A extraction."""
    parser = argparse.ArgumentParser(
        description="Extract MD&A sections from 10-K filings with in-place table preservation"
    )
    ...
    parser.add_argument("--preserve-tables", action="store_true", default=True,
                       help="Preserve tables in original positions (default: True)")
    ...
    try:
        ...
        logger.info(f"Table preservation mode: {'ENABLED' if args.preserve_tables else 'DISABLED'}")

        # Process ZIP files with improved processor
        processor = ImprovedZipProcessor(
        ...
        # Report on table preservation
        if args.preserve_tables:
            logger.info(f"Tables preserved in-place within MD&A text")
...
class ImprovedZipProcessor(ZipProcessor):
    """Improved ZIP processor with in-place table preservation."""

    def __init__(self, output_dir: Path, raw_dir: Path, mdna_only: bool = True,
                 preserve_tables: bool = True, delete_raw_after_processing: bool = True):
        ...
        logger.info(f"Initialized processor with table preservation: {preserve_tables}")

    def process_zip_file(self, zip_path: Path, cik_filter=None) -> Dict[str, any]:
        """Process ZIP file with improved extraction."""
                    ...
                    try:
                        ...
                        with zf.open(file_name) as source:
                            ...
                            if cik_match:
                                ...
                                # Process for MD&A extraction with improved extractor
                                result = self.extractor.extract_from_file(raw_file_path)

                                if result:
                                    stats["processed"] += 1
                                    # Count tables preserved
                                    if result.tables:
                                        stats["tables_preserved"] += len(result.tables)
                                    logger.info(f"  ✅ Extracted MD&A from: {file_name}")
                                    if result.tables:
                                        logger.info(f"     Preserved {len(result.tables)} tables in-place")
                                else:
                                    stats["failed"] += 1
        ...
        logger.info(f"ZIP complete: {stats['processed']} MD&A sections extracted, "
                   f"{stats['tables_preserved']} tables preserved, {stats['filtered_out']} filtered")
        return stats

    def process_directory(self, input_dir: Path, cik_filter=None) -> Dict[str, any]:
        """Process directory with improved statistics."""
        overall_stats = {
            "total_zips": 0,
            "total_files": 0,
            "processed": 0,
        ...
        for zip_path in sorted(zip_files):
            ...
            overall_stats["tables_preserved"] += stats.get("tables_preserved", 0)

        logger.info(f"Total tables preserved in-place: {overall_stats['tables_preserved']}")
